 ...
 * @author Omistaja
 */
public class DataLengthWithSpacesFieldMapperImpl implements RequestResponseMapper<AnalyzeRequest, AnalyzeResponse> {

    @Override
    public void map(AnalyzeRequest requestData, AnalyzeResponse responseObject) {
        String requestText = requestData.getText();
        responseObject.setTextLengthWithSpaces(requestText == null ? 0 : requestText.length());
    }    
}
