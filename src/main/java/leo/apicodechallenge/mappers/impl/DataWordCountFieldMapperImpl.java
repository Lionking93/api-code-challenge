 ...
 * @author Omistaja
 */
public class DataWordCountFieldMapperImpl implements RequestResponseMapper<AnalyzeRequest, AnalyzeResponse> {
    ...
    public void map(AnalyzeRequest requestData, AnalyzeResponse responseObject) {
        ...
        if (requestText != null) {
            String dataWithoutSpaces = requestText.trim();
            
            if (dataWithoutSpaces.length() > 0) {
                String[] dataParts = dataWithoutSpaces.split("\\s+");
                wordCount = dataParts.length;
            }
        }
        
        responseObject.setWordCount(wordCount);
    }
